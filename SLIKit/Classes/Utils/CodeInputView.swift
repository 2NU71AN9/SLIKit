...
@IBDesignable
final public class CodeInputView: UIView, UIKeyInput {
    ...
    private var xPosition: CGFloat {
        ((frame.width - (baseWidth * CGFloat(numberOfSlots))) / 2) + (CGFloat((currentSlot - 1)) * CGFloat(baseWidth))
    }
    
    private var frameRect: CGRect {
        CGRect(x: xPosition, y: 0, width: baseWidth, height: frame.height)
    }
